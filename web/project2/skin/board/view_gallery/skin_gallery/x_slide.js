...
function galleryMode(current) {
    currentPosition = Number(galleryTable.style.marginLeft.slice(0, -2));
    currentGallery = function() {
        if (currentPosition == current) clearInterval(galleryInterval);
            ...
            galleryTable.style.marginLeft = currentPosition + "px";
        }
    }
...
function galleryChange() {
	currentSize = galleryDiv.offsetWidth; 
...
onresize = function() {
	...
	galleryWidthPixel = Math.floor(galleryDiv.offsetWidth) + "px"; 
	galleryDiv.style.width = imageTable.style.width = galleryWidthPixel;
...
function galleryHtml() {
    document.write("<div id=totalDiv><div style=display:flex;justify-content:center><div id=galleryDiv><table id=galleryTable cellpadding=0 cellspacing=0 style=width:" + imageTotal * 100 + "%;height:100%;table-layout:fixed;margin-left:0px>");
    for (img = 1; img <= imageTotal; img++) document.write("<td style=background-image:url(" + this['image_' + img] + ");background-size:cover;background-position:center></td>");
...
soundBtn.onclick = function() {
	if (soundMp3 != "") {
		if (soundBtn.src == playImage) {
			galleryAudio.play();
			soundBtn.src = stopImage;
		}
		else {
			galleryAudio.pause();
			soundBtn.src = playImage;
		}
	}
}